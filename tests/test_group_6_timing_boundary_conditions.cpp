...
TEST_F(TimingBoundaryConditionsTest, HoldTimeoutBoundary1msBefore) {
    ...
    keyboard.release_key_at(TAP_DANCE_KEY, 199);    // t=199ms (1ms before hold timeout)
    keyboard.wait_ms(200);                      // t=399ms (tap timeout)
...
TEST_F(TimingBoundaryConditionsTest, HoldTimeoutBoundaryExactlyAt) {
    ...
    keyboard.press_key_at(TAP_DANCE_KEY, 0);        // t=0ms
    keyboard.wait_ms(200);                      // t=200ms (exactly at hold timeout)
...
TEST_F(TimingBoundaryConditionsTest, HoldTimeoutBoundaryJustAfter) {
    ...
    keyboard.press_key_at(TAP_DANCE_KEY, 0);        // t=0ms
    keyboard.release_key_at(TAP_DANCE_KEY, 201);    // t=201ms (1ms after timeout)
...
TEST_F(TimingBoundaryConditionsTest, TapTimeoutBoundarySequenceReset) {
    ...
    keyboard.wait_ms(200);                      // t=300ms (tap timeout expires - sequence resets)
...
TEST_F(TimingBoundaryConditionsTest, TapTimeoutBoundarySequenceContinuation) {
    ...
    keyboard.press_key_at(TAP_DANCE_KEY, 0);        // t=0ms
    keyboard.release_key_at(TAP_DANCE_KEY, 100);    // t=100ms
    keyboard.press_key_at(TAP_DANCE_KEY, 199);      // t=299ms (1ms before tap timeout)
    keyboard.release_key_at(TAP_DANCE_KEY, 50);     // t=349ms
    keyboard.wait_ms(200);                      // t=549ms
...
TEST_F(TimingBoundaryConditionsTest, RaceConditionHoldVsTapTimeout) {
    ...
    keyboard.press_key_at(TAP_DANCE_KEY, 0);        // t=0ms
    keyboard.release_key_at(TAP_DANCE_KEY, 100);    // t=100ms (start tap timeout)
    // Next press at exactly when both timeouts could occur
    keyboard.press_key_at(TAP_DANCE_KEY, 200);      // t=300ms (tap timeout + hold start)
    keyboard.wait_ms(200);                      // t=500ms (hold timeout)
    keyboard.release_key_at(TAP_DANCE_KEY, 500);    // t=500ms
...
TEST_F(TimingBoundaryConditionsTest, RaceConditionStrategyVsTimeout) {
    ...
    keyboard.press_key_at(TAP_DANCE_KEY, 0);          // t=0ms
    keyboard.press_key_at(INTERRUPTING_KEY, 199);     // t=199ms (1ms before hold timeout)
    keyboard.release_key_at(INTERRUPTING_KEY, 2);     // t=201ms (complete cycle after timeout)
    keyboard.release_key_at(TAP_DANCE_KEY, 49);       // t=250ms
...
TEST_F(TimingBoundaryConditionsTest, RapidSequenceTimingSubTimeoutWindows) {
    ...
    keyboard.press_key_at(TAP_DANCE_KEY, 20);       // t=20ms
    keyboard.release_key_at(TAP_DANCE_KEY, 30);     // t=30ms
    keyboard.press_key_at(TAP_DANCE_KEY, 40);       // t=40ms
...
TEST_F(TimingBoundaryConditionsTest, MultipleTimeoutWindowsSequenceChain) {
    ...
    keyboard.press_key_at(TAP_DANCE_KEY, 245);      // t=245ms (within first tap timeout)
    keyboard.release_key_at(TAP_DANCE_KEY, 295);    // t=295ms
    // Wait near second tap timeout, then continue
    keyboard.press_key_at(TAP_DANCE_KEY, 490);      // t=490ms (within second tap timeout)
    keyboard.release_key_at(TAP_DANCE_KEY, 540);    // t=540ms
    keyboard.wait_ms(200);                      // t=740ms
...
TEST_F(TimingBoundaryConditionsTest, TimeoutAccumulationLongSequence) {
    ...
    for (int i = 0; i < 5; i++) {
        keyboard.press_key_at(TAP_DANCE_KEY, i * 180);      // t=i*180ms
        keyboard.release_key_at(TAP_DANCE_KEY, i * 180 + 50); // t=i*180+50ms
        keyboard.wait_ms(130);                          // t=i*180+180ms
    }
...
TEST_F(TimingBoundaryConditionsTest, ZeroDurationEdgeCases) {
    ...
    keyboard.press_key_at(TAP_DANCE_KEY, 0);        // t=0ms
    keyboard.release_key_at(TAP_DANCE_KEY, 0);      // t=0ms (zero duration)
    keyboard.press_key_at(TAP_DANCE_KEY, 0);        // t=0ms (immediate second press)
    keyboard.release_key_at(TAP_DANCE_KEY, 100);    // t=100ms
...
TEST_F(TimingBoundaryConditionsTest, TimeoutBoundaryWithStrategyIntegration) {
    ...
    keyboard.press_key_at(INTERRUPTING_KEY, 200);     // t=200ms (exactly at timeout)
    keyboard.release_key_at(INTERRUPTING_KEY, 201);   // t=201ms
...
TEST_F(TimingBoundaryConditionsTest, ComplexTimingScenarioMixedBoundaries) {
    ...
    keyboard.press_key_at(TAP_DANCE_KEY, 200);        // t=200ms (2nd tap, exactly at first timeout)
    keyboard.press_key_at(INTERRUPTING_KEY, 399);     // t=399ms (1ms before second hold timeout)
    keyboard.release_key_at(INTERRUPTING_KEY, 401);   // t=401ms (complete cycle after timeout)
    keyboard.release_key_at(TAP_DANCE_KEY, 450);      // t=450ms
