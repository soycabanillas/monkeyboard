...
// Simple Key Replacer
// Objective: Verify basic key replacer functionality
TEST_F(KeyReplacer, SimpleKeyReplacer) {
    const uint16_t ONE_SHOT_KEY = 100;
    const uint16_t OUTPUT_KEY1 = 101;
    const uint16_t OUTPUT_KEY2 = 102;

    static const platform_keycode_t keymaps[1][1][1] = {{{ ONE_SHOT_KEY }}};
    platform_layout_init_2D_keymap((const uint16_t*)keymaps, 1, 1, 1);


    size_t number_of_pairs = 1;
    pipeline_key_replacer_global_config_t* global_config = static_cast<pipeline_key_replacer_global_config_t*>(malloc(sizeof(*global_config)));
